...
For plotting tasks, honor the current explicit Python/R choice, then the established project workflow or language-specific input. Otherwise read the saved preference with `scripts/nature_figure_backend.py get`.

If none determines the backend, use Python when its required plotting runtime is available; use an available R workflow when Python is unavailable. State this task-local choice briefly and proceed. Ask only if the choice materially changes a required deliverable or conflicts with the project's contract. Once a backend is selected, the missing-runtime rule below applies.

Save a persistent preference with `scripts/nature_figure_backend.py set python` or `set r` when the user explicitly establishes or changes a default. Do not turn an inferred task-local choice into a saved user preference.
