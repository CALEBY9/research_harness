...
The source preflight checks syntax, typography, colors, export hints, dimensions,
sampling, transformations and backend use. A dynamic filename is unresolved,
not proof of a missing export: inspect the actual requested artifacts. PNG is
sufficient when the delivery contract requests PNG; explicit TIFF submission
requirements still apply. The PDF audit scans actual `Tf` text-size operators.

For explicit limits on numeric plots, compare all plotted coordinates with the
axis bounds before export (`scripts/figure_safety.py:assert_visible_points` or
equivalent). Account for intentional clipping rather than silently losing points.
Static source checks cannot establish data visibility or replace the rendered
inspection of labels, uncertainty extents, and marker edges.
