...
name: nature-data
description: >-
  Draft or audit Data Availability statements, repository and accession plans, dataset
  citations, restricted-data wording, source-data packages, and FAIR metadata. Use for
  数据可用性声明、代码或数据共享、数据仓库选择, or dataset citation work for journal submissions.
metadata:
...
For an existing submission package or any statement that data/code are
available, deposited, accessible, or release-ready, load
`../nature-shared/core/evidence-authority.md`. Verify the actual repository
record, identifier, access route, licence/restriction, and current artifact
binding. A drafted statement, planned deposit, local file, or QA pass is not
evidence that public or reviewer access exists. Keep unresolved deposits or
identifiers as explicit placeholders rather than submission-ready claims.
