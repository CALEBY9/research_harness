...
- If the draft is structurally rough and the requested scope permits
  restructuring, reconstruct the logic first and the prose second. For
  `LANGUAGE_ONLY`, preserve the existing logic and improve its expression.
- Preserve epistemic strength. Do not add defensive hedges merely because the
  passage has not undergone a separate evidence audit, and do not silently
  strengthen claims. Language polishing preserves claim state; it does not
  certify claim validity.
- For a short supplied passage, keep terminology checking internal and surface
  only a collision that could change meaning. Build and expose a Terminology
  Ledger only for whole-manuscript or multi-section consistency work, visible
  drift, or an explicit terminology-audit request. Do not introduce synonyms
  merely to vary the prose. See
