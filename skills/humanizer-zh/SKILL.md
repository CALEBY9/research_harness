...
name: humanizer-zh
description: |
  改写带有公式化 AI 写作痕迹的中文文本，使表达更自然，同时保持事实、主张和语气。
  适用于用户明确要求“去 AI 味”“人性化”或按其个人文风改写中文文本；英文文本使用 humanizer:humanizer。
...
3. **保留含义** - 保留全部事实、主张、数字、日期、引文、引用及其限定条件
4. **维持语调** - 匹配预期的语气（正式、随意、技术等）
5. **匹配作者声音** - 仅在体裁和原文需要时保留个性，不凭空添加观点或经历

## 事实与学术文本边界

- 目标是改善写作质量，不是规避 AI 检测器，也不对文本作者身份作判断或保证。
- 不得编造原文或用户没有提供的事实、姓名、数字、日期、例子、引文、引用、研究结果或来源。缺少具体信息时，使用更朴素的表达；若信息对改写不可缺少，单独向用户指出。
- 处理论文、proposal、报告或其他事实性文本时，保持术语、因果方向、模态词、hedges、scope conditions、claim strength、citation function 和段落功能不变。
- 不因词语出现在警示列表中就机械删除。先判断它是否承担技术含义、证据边界、必要限定或作者刻意的文风功能。
- 用户提供个人写作样本时，优先匹配该样本的句长、用词、标点和节奏；没有样本时才使用下列通用规则。
...
默认只提供可直接使用的重写后文本。仅当用户要求诊断、对照或修改说明时，再单独提供简短总结；不得把过程说明混入论文、报告、正式信件或其他 clean final。
