...
description: >-
  Draft or audit reviewer-separated response letters, rebuttals, revision cover letters,
  and marked manuscript excerpts without leaking between blind reviewers. Use for
  审稿意见回复、修回信、返修邮件、rebuttal, or revision correspondence; not for
...
Whenever the package includes manuscript edits, version selection, or a claim
that an action is complete, also load
`../nature-shared/core/evidence-authority.md`. Bind the exact decision letter,
manuscript version or package, and editable spans before editing. A newer file,
redline, or passed format check is not automatic canonical authority. Use
`VERIFIED_DONE` only after the claimed change is matched to the supplied
artifact; otherwise keep the item reported-but-unverified, pending, or blocked.
Keep the clean manuscript and reviewer-facing letters separate from the
author-side tracker, line map, and change log.
...
`qa-checklist.md` and `package-consistency-audit.md` are complementary and both apply to a final package: the first asks whether the response is complete, honest, and well-toned; the second asks whether the marked manuscript, the clean manuscript, and the letter actually agree with each other after editing. For a LaTeX package, run `scripts/check_package_consistency.py` after a completed editing batch and verify the final package before delivery. A passed check on the same unchanged manuscript/letter/clean/marked files also satisfies the final check; do not repeat it solely because the task is ending. Later text, numbering, pagination or artifact changes invalidate affected quotes/locations and require the relevant consistency checks again.
