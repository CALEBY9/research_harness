...
description: >-
  Audit, revise, or draft manuscript statistical reporting, including p values, confidence
  intervals, sample size, replicates, randomization, multiple comparisons, assumptions,
  Results wording, and figure legends. Use for 统计审查、统计分析小节、p值、样本量、
  效应量, or reviewer statistics comments.
---
...
4. **Map claims to analyses.** For each result claim, record the comparison/model, test family, assumptions, correction strategy, effect estimate, uncertainty, and exact p-value policy. When completed statistics will license manuscript claims, load `../nature-shared/core/evidence-authority.md` and record the result locator, claim tested, confirmatory condition, support disposition, strongest allowed interpretation, interpretation not licensed, and missing check. Missing or unperformed support is `HOLD_FOR_EVIDENCE`; direct contradiction is `REJECT_OR_REMOVE`.
