...
description: |
  Run recurring literature discovery from multi-source search through scoring, fine reading, formatted delivery, and archival. Use for 定时文献追踪、自动文献监测, or Feishu/Telegram literature pipelines; not for a one-off search.
license: MIT
metadata:
  author: Jiahao8595
  hermes:
...
- **Explicit automation authority**: Creating a cron/scheduled task, sending to
  Feishu/Telegram/email, or changing an external archive requires the user's
  explicit approval of the target, schedule, and write surface. A dry run or
  generated config is not proof that delivery occurred.
- **Evidence boundary**: `Full-text`, `Abstract only`, and `Metadata only`
  describe access, not support. Claim-bearing synthesis must verify the
  original source and follow
  `../nature-shared/core/evidence-authority.md` before entering manuscript prose.
