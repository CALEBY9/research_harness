...
> **SI confirmation gate.** Resolve SI choice before any download: reuse this batch's explicit SI or 正文-only request; otherwise ask once, grouping any unresolved output-folder or browser-fallback authorization with it. Run with exactly one of `--si` or `--no-si`; neither flag means `si_confirmation_required` and no output directory is created.
...
7. If no lawful OA full text is found, mark `oa_not_found`. For a supported publisher whose API already failed, require this batch's explicit Web Access authorization, reusing it if already given. For another publisher, continue to Web Access. If `--route open_access` was explicitly requested, stop after the OA result.

### Publisher API Credentials

User-run local credential commands, only when the route needs configuration:
...
Do not ask for API keys in chat. Receiving a key does not authorize saving it:
follow the global credential rules and do not echo or persist it. If configuration
is needed, the user completes it through the supported local hidden-input path.
IEEE Metadata API access is not paid full-text access; require the issued Full-Text
Access endpoint/template before treating IEEE as downloadable through the API.
...
**Verification handoff:** For CAPTCHA, sliders, robot checks, Cloudflare or publisher
bot verification, and identity-bearing prompts such as QR approval, SMS/OTP,
passkey, hardware key, or two-factor authentication, use the user-handoff rules in
`references/institutional-browser-workflow.md`. Keep the tab open; never request
credentials or codes in chat. Distinguish ordinary navigation from verification
using page context, not the button label `Continue` or `Verify` alone.
...
Do not ask the user to paste institutional passwords, database passwords, OTP codes, recovery codes, or session tokens into chat or terminal. If the user offers one of those identity-bearing secrets, decline and use the handoff-login workflow instead. Publisher API keys follow the credential rules above.
...
Before downloading, verify the applicable technical conditions with tools; user authorization is separate from environment verification.
...
8. The user has specified or approved the target output folder for this batch.
...
- hand verification controls to the user under the verification-handoff rule; pause the affected tab on institutional login expiry or an unusual/security-sensitive prompt
...
`verification_auto_passed` and `verification_auto_failed` are historical status
values for reading earlier manifests, not authorization for new automatic attempts.
...
Use `publisher_verification_waiting_user` or `sciencedirect_robot_check` for a
publisher verification challenge awaiting the user. Neither is a final download failure.
