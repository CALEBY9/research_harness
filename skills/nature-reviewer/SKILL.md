...
description: >-
  Produce an evidence-grounded pre-submission peer-review report from the referee perspective,
  with Major Concerns, Minor Comments, and blocking flags. Use for 模拟审稿、投稿前预审、
  审稿人视角评估, or manuscript critique; not for author rebuttals.
---
...
- For a versioned manuscript, claim-licensing dispute, or readiness decision,
  load `../nature-shared/core/evidence-authority.md`. Do not infer authority
  from recency, release labels, or QA alone, and do not treat an available
  artifact as support until its relevance and boundary are checked.
...
Delegation is not implied by selecting this skill. Use subagents only when the
user explicitly authorizes delegation and the active instruction layer permits
it. Otherwise generate one frozen reviewer report per separate invocation, or
state that mutual blindness cannot be guaranteed and provide a bounded
single-context review. Never label shared-context output as mutually blind.
