...
Resolve the intake checklist from materials and session context first. Group related blocking questions unless the user requests one-at-a-time discussion.
...
**Rule**: ask only about missing information that changes the writing path and cannot reasonably be inferred. Use established context directly; do not turn it into a confirmation question.
